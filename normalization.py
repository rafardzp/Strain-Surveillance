...
class ScaleNormalizer(BaseEstimator, TransformerMixin):
    ...
    def _normalize_spectrum(self, spectrum):
        ...
        maldi = spectrum[:, np.newaxis] * np.array([1, scaling])[np.newaxis, :]
        return maldi[:,1]
    ...
    def transform(self, X):
        maldis = [self._normalize_spectrum(spectrum) for spectrum in X]
        return np.vstack(maldis)
